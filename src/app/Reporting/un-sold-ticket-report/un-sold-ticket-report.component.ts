...
export class UnSoldTicketReportComponent implements OnInit {
  ...
  exportToPDF(): void {
    ...
    const generatedBy = this.currentUserFullName; // Replace with dynamic value if needed
    
    if (chartElement && tableElement) {
      // Capture both chart and table as images
      Promise.all([
        ...
        const chartWidth = 180; // Adjust as necessary
        const chartHeight = (chartCanvas.height * chartWidth) / chartCanvas.width;
        pdf.addImage(chartImgData, 'PNG', 15, 40, chartWidth, chartHeight);
  
        // Add table image below the chart
        const tableImgData = tableCanvas.toDataURL('image/png');
        const tableYPosition = 40 + chartHeight + 10; // Adjust position to fit after the chart
        const tableWidth = 180; // Adjust as necessary
